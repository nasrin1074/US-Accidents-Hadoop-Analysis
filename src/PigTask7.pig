...
valid = FILTER accidents BY Sunrise_Sunset IS NOT NULL
    AND Sunrise_Sunset != '';

grouped = GROUP valid BY Sunrise_Sunset;

daynight_count = FOREACH grouped GENERATE
    group AS Sunrise_Sunset,
    COUNT(valid) AS Accident_Count;

ordered = ORDER daynight_count BY Accident_Count DESC;

STORE ordered INTO '/usaccidents/pig/task7_day_night'
USING PigStorage(',');
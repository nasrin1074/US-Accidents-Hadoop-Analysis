...
valid_state = FILTER accidents BY State IS NOT NULL AND State != '';

grouped = GROUP valid_state BY State;

state_count = FOREACH grouped GENERATE
    group AS State,
    COUNT(valid_state) AS Accident_Count;

ordered = ORDER state_count BY Accident_Count DESC;

STORE ordered INTO '/usaccidents/pig/task2_state_count'
USING PigStorage(',');